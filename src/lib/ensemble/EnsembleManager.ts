...
export function videoRppgStableFaceBoost(biometric: BiometricInputs, votes: SpecialistVote[]): number {
...
export function ForensicAnalyst(kind: EvidenceKind, input: ForensicInputs, weights: EnsembleWeights): SpecialistVote {
	...
	if (kind === 'image') {
		...
		const cameraPipelineLikely = !strongSynth && (localCv > 0.34 || localPeak > 1.72 || (nm > 0.033 && nu < 0.52));

		// --- Ruido “demasiado limpio” (más estricto; no castigar JPEG con grano real) ---
		const tooClean =
			!cameraPipelineLikely && ((nm > 0 && nm < 0.022 && nu > 0.52) || (nm > 0 && nm < 0.028 && nu > 0.62));
		...
		const rsLooksLikeCameraNr =
			!strongSynth && ((localCv > 0.48 && nm < 0.028) || (localCv > 0.51 && nm < 0.036 && rs > 0.55));
		...
		const waRealBlock =
			nu < 0.33 && localCv > 0.38 && localCv < 0.52 && nm > 0.052 && nm < 0.085 && ep < 0.32 && fp > 0.38;
		const mobileFlatElaBlock = nu > 0.45 && fp > 0.32 && localCv < 0.33 && nm > 0.055 && nm < 0.1;
		...
		const edgeGenLike =
			Boolean(input.frequencySynthetic) && li > 0.83 && ep > 0.38 && ep < 0.52 && nm < 0.072 && fp < 0.22;
		...
		const studioExportLikely =
			Boolean(input.frequencySynthetic) && ep > 0.48 && ep < 0.58 && li > 0.88 && nu < 0.21 && rs < 0.52 && fp < 0.12;
...
export function BiometricAnalyst(kind: EvidenceKind, input: BiometricInputs, weights: EnsembleWeights): SpecialistVote {
	...
	if (rf >= 14 && bc === 0 && !input.blinkWarning && minC >= 0.9 && maxJ <= 0.021 && !input.suspiciousLowConfidence) {
		score += 46;
	}
...
export function RppgAnalyst(
	...
	if (res.rhythmicPulseLikely) {
		...
		notes.push(`Pico espectral plausible (~${res.estimatedBpm ?? '?'} lpm, prominencia ${res.prominence.toFixed(2)}).`);
	} else {
