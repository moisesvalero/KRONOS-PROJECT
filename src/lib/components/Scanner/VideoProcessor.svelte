<script lang="ts">
	...
	function buildVideoFeaturesForExport(
		...
		if (exportExtras.rppgPrecomputed) adv.rppg = { precomputed: exportExtras.rppgPrecomputed };
		if (exportExtras.containerPrecomputed) adv.container = { precomputed: exportExtras.containerPrecomputed };
	...
	let animatedLogIndex = $derived(
		scan.phase === 'ANALYZING'
			? Math.min(scan.logs.length, Math.floor((displayMillis / ANALYSIS_DURATION_MS) * scan.logs.length) + 1)
	...
	let metric1Ok = $derived(
		isAudioModeUi
			? !scan.warnings?.includes('Posible edición por cortes (Audio Continuidad)')
			: !scan.warnings?.includes('Patrón de Parpadeo Sintético')
	);
	let metric2Ok = $derived(
		isAudioModeUi ? !scan.warnings?.includes('Naturalidad acústica atípica (posible TTS)') : true
	);
	...
	let metric2ScoreUi = $derived(
		isAudioModeUi
			? metric2Ok
				? 0.88
				: 0.42
			: clamp01((isVideoModeUi ? lastFrameScore : 0.85) - (isVideoModeUi ? lastFrameJitter * 2.2 : 0.05))
	);
	...
	let dataScoreUi = $derived(
		clamp01(
			0.92 -
				...
				(scan.warnings?.includes('Origen: Software de Terceros detectado. Integridad de Cámara: Comprometida')
					? 0.24
					: 0) -
	...
	function classifyLink(url: string) {
		...
		const isDirectMedia =
			/\.(mp4|webm|mov|mkv|avi|m4v|3gp|mp3|wav|m4a|jpg|jpeg|png|webp|gif|bmp|tif|tiff)(\?|#|$)/i.test(path);
	...
	async function runWorkerAudit(file: File, doMediaInfo: boolean) {
		...
		const p = new Promise<{ hashHex: string; mi: { thirdParty: boolean; encoderHints: string[] } }>(
			(resolve, reject) => {
		...
		const walk = (v: unknown) => {
			...
			if (typeof v === 'string') {
				...
				if (
					...
					(/adobe|premiere|after effects|davinci|resolve|capcut|final cut|kinemaster|vlc/i.test(s) ||
			...
			try {
					...
					for (let tx = 0; tx < tilesN; tx++) {
						...
						for (let yy = y0 + 1; yy < y1; yy += 2) {
							...
							for (let xx = x0 + 1; xx < x1; xx += 2) {
								...
								const lap = -4 * center + grayA[i - 1] + grayA[i + 1] + grayA[i - w] + grayA[i + w];
								s += Math.abs(lap);
								c++;
							}
						}
				...
				for (let yy = 1; yy < h - 1; yy += 2) {
					for (let xx = 1; xx < w - 1; xx += 2) {
						...
						const lap = -4 * grayA[i] + grayA[i - 1] + grayA[i + 1] + grayA[i - w] + grayA[i + w];
	...
	async function analyzeTextLLM(text: string) {
		...
		const sentVar =
			sentCount > 1 ? sentenceLens.reduce((acc, x) => acc + (x - sentMean) * (x - sentMean), 0) / (sentCount - 1) : 0;
		...
		const connectorScore = Math.max(
			0,
			Math.min(1.6, connectorPerSentence * 10 * (0.4 + uniqueRatio) + repetitionBoost * 0.25)
		);
	...
	function sobelFaceStatsAndMag2(imageData: ImageData, topBandFrac: number, bottomBandFrac: number) {
	...
	function analyzeMaskEdgesAndHalo(video: HTMLVideoElement, pts: { x: number; y: number }[]) {
		...
		const edgeCanvas = edgeOverlayCanvas ?? (typeof document !== 'undefined' ? document.createElement('canvas') : null);
		...
		// Neck ROI (stats only)
		const neckCanvas = neckEdgeCanvas ?? (typeof document !== 'undefined' ? document.createElement('canvas') : null);
		if (!neckCanvas) {
		...
		const halo = ratioFaceInterior > 1.45 && ratioFaceNeck > 1.7 && ratioStd > 1.15 && faceStats.boundaryMean > 15;
	...
	async function analyzeFramesReal() {
		...
		try {
			while (analyzedMs < windowMs && performance.now() - start < maxWallMs) {
				...
				const result = await faceapi.detectSingleFace(videoRef, options).withFaceLandmarks(true);
				detectCount += 1;
	...
	async function onLinkFileInput(event: Event) {
		...
		const isImage = file.type?.startsWith('image/') || /\.(png|jpe?g|webp|gif|bmp|tiff?)$/i.test(name);
		const isVideo = file.type?.startsWith('video/') || /\.(mp4|webm|mov|mkv|avi|m4v|3gp)$/i.test(name);
		const isAudio = file.type?.startsWith('audio/') || /\.(mp3|wav|m4a|aac|ogg|opus|flac)$/i.test(name);
	...
	async function runTextAnalysis() {
		...
		try {
			...
			if ((result as any).sentCount >= 4 && (result as any).sentenceLenCv < 0.12)
				warnings.push('Varianza de longitud de frases inusualmente baja (proxy)');
			...
			if (mediaKind === 'audio') {
				try {
					...
					if ((res as any).digitalSilenceGaps >= 2)
						warningsAudio.push('Silencio digital absoluto entre segmentos (proxy)');
				...
				try {
					...
					const rppgAnalysis = analyzeRppgGreenSeries((frameResult as any).rppgGreenSamples ?? [], rppgRate);
					...
					const ens = ensemble.evaluate({
						...
						forensic: {
							...
							videoPortraitSyntheticHint: Boolean(videoExportFeatures?.forensic?.videoPortraitSyntheticHint ?? false),
				...
				if (imageNoiseMean > 0 && imageNoiseMean < 0.018 && imageNoiseUniformity > 0.72)
				...
				completeScan({
					...
					logsExtra: [
						...(((imgResult as any).__kronosEnsembleVotes as string[] | undefined) ?? []).map(
							(s) => `ENSEMBLE_VOTE: ${s}`
						),
	...
	function drawOverlay() {
		...
		ctx.strokeStyle = scan.verdict === 'ALERTA ROJA' ? 'rgba(230, 57, 70, 0.7)' : 'rgba(0, 229, 255, 0.7)';
...
<section class="processor">
	<div class="main-card">
			...
			<div class="link-panel">
				...
				<div class="link-form">
					...
					<div class="link-actions">
						...
						<div class="link-upload">
							<input id="link-file-input" type="file" accept="video/*,image/*,audio/*" on:change={onLinkFileInput} />
				...
				<div class="link-guidance">
					...
					<ul>
						...
						<li>
							Si solo tienes un enlace público, KRONOS puede registrar el enlace como referencia, pero la auditoría
							profunda requiere el archivo.
						</li>
					</ul>
				</div>
			</div>
		...
		<footer>
			...
			<button type="button" class="btn-ghost" on:click={clearSession} disabled={isBusy}
				>{$t('scanner.buttons.reset')}</button
			>
		</footer>
	</div>
		...
		<div class="validation">
			...
			<div class="val-rows">
				...
				<p>
					...
					<strong class="mono">{scan.riskScore ? `${Math.round(riskScoreSmooth)}` : '-'}</strong>
				</p>
				<p class="row-file">
					<span>{$t('scanner.details.file')}</span> <strong class="mono file-val">{selectedFile?.name ?? '—'}</strong>
				</p>
				<p>
					<span>{$t('scanner.details.weight')}</span>
					<strong class="mono">{selectedFile ? `${(selectedFile.size / 1024 / 1024).toFixed(2)} MB` : '—'}</strong>
				</p>
